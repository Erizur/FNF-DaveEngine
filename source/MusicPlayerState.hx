	...
	override function create()
	{
		...
		for (i in 0...initSonglist.length)
		{
			...
			songs.push(new PlaySongMetadata(splitstring[1], splitstring[0] == "external", splitstring[2], splitstring[3] == "bad",
				splitstring[1] != 'tutorial'));

			if (splitstring[0] != "external" && splitstring[1] != 'tutorial') // remove this later
			{
		...
		{
			var songText:Alphabet = new Alphabet(0, 0, songs[i].songName + (songs[i].hasVocals ? "" : ((songs[i].songName != 'tutorial') ? "-Inst" : "")),
				true, false);
